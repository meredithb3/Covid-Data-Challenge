...
```{r ridemodelresid}
couty_deaths <- county_deaths %>%
  mutate(predicted = predict.lm(final), resid = residuals(final))
```

```{r predict-resid}
ggplot(data = data, mapping = aes(x = predicted, y = resid)) + 
  geom_point() + 
...
Now that we have determined that our model is satisfactory, we can begin to provide an interpretation of the model. 
