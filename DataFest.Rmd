...
full_model <- lm(logDeathsPC ~ cases + Rural.urban_Continuum_Code_2003 + Urban_Influence_Code_2003 
                 + Rural.urban_Continuum_Code_2013 + Urban_Influence_Code_2013 + POVALL_2018 
                 + PCTPOVALL_2018 + POV017_2018 + PCTPOV017_2018 + POV517_2018 + PCTPOV517_2018 
                 + MEDHHINC_2018 + pop_2015 + num_white + num_black + num_hispanic + num_asian 
                 + num_other + num_nonwhite + perc_white + perc_black + perc_hispanic 
                 + perc_nonwhite, data = county_deaths)
...
final_model <- lm(logDeathsPC ~ cases + Rural.urban_Continuum_Code_2013 + 
                    ...
                    pop_2015 + num_asian + perc_white + perc_black, data = county_deaths)
...
```{r pct-pov-0-17}
ggplot(data = county_deaths, aes(x = PCTPOV017_2018)) +
  geom_histogram(color = "Black", fill = "Light Blue", bins = 20) + 
  labs(title = "Distribution of Percent Poverty in Ages 0-17", x = "Percent Poverty", y = "Count")
```

```{r pct-pov-5-17}
ggplot(data = county_deaths, aes(x = PCTPOV517_2018)) +
  geom_histogram(color = "Black", fill = "Light Blue", bins = 20) + 
  labs(title = "Distribution of Percent Poverty in Ages 5-17 in Families", x = "Percent Poverty",
       y = "Count")
```

```{r median-househould-income}
ggplot(data = county_deaths, aes(x = MEDHHINC_2018)) +
  geom_histogram(color = "Black", fill = "Light Blue", bins = 30) + 
  labs(title = "Distribution of Median Household Income (2018)", x = "Median Household Income (2018)",
       y = "Count")
```
...
```{r num_asians}
ggplot(data = county_deaths, aes(x = num_asian)) +
  geom_histogram(color = "Black", fill = "Light Blue", bins = 20) + 
  labs(title = "Distribution of Number of Asian Persons", x = "Number of Asian Persons", y = "Count")
```
...
The distributions for `cases`, `pop_2015`, `num_asian`, `perc_white`, and `perc_black` all appear to be extremely skewed, so they do not satisfy the normality condition. To correct this, we will try taking the log of each of these variables.
...
```{r log-num_asians}
ggplot(data = county_deaths, aes(x = log(num_asian))) +
  geom_histogram(color = "Black", fill = "Light Blue", bins = 20) + 
  labs(title = "Distribution of Log Number of Asian Persons", x = "Log Number of Asian Persons", y = "Count")
```
...
ggplot(data = county_deaths, aes(x = PCTPOV017_2018, y = logDeathsPC)) +
  geom_point(color = "black") +
  labs(title = "Log Deaths/Capita vs Percent Poverty in Ages 0-17", y = "Log Deaths/Capita", 
       x = "Percent Poverty in Ages 0-17")
```
        
```{r logdeathsPC-pct-pov-5-17}
ggplot(data = county_deaths, aes(x = PCTPOV517_2018, y = logDeathsPC)) +
  geom_point(color = "black") +
  labs(title = "Log Deaths/Capita vs Percent Poverty in Ages 5-17 in Families", y = "Log Deaths/Capita", 
       x = "Percent Poverty in Ages 5-17 in Families")
```        

```{r logdeathsPC-medhhinc}
ggplot(data = county_deaths, aes(x = MEDHHINC_2018, y = logDeathsPC)) +
  geom_point(color = "black") +
  labs(title = "Log Deaths/Capita vs Median Household Income (2018)", y = "Log Deaths/Capita", 
       x = "Median Household Income (2018)")
```   

```{r logdeathsPC-logpopulation}
county_deaths <- county_deaths %>%
  mutate(log_pop_2015 = log(pop_2015))
...
```{r logdeathsPC-lognum_asian}
county_deaths <- county_deaths %>%
  mutate(log_num_asian = log(num_asian))

ggplot(data = county_deaths, aes(x = log_num_asian, y = logDeathsPC)) +
  geom_point(color = "black") +
  labs(title = "Log Deaths/Capita vs Log Number of Asian Persons", y = "Log Deaths/Capita", 
       x = "Log Number of Asian Persons")
```
...
final <- lm(logDeathsPC ~ logCases + Rural.urban_Continuum_Code_2013 + 
                    PCTPOV017_2018 + PCTPOV517_2018 + MEDHHINC_2018 + 
                    log_pop_2015 + log_num_asian + perc_white + perc_black, data = county_deaths)
