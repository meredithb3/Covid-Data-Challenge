...
For COVID data, download the file called us-states.csv from this url:
https://github.com/nytimes/covid-19-data/blob/master/us-states.csv
For demographic data, download files called stco-mr2010-1.csv and stco-mr2010_mt_wy.csv from this url: https://www2.census.gov/programs-surveys/popest/datasets/2010/modified-race-data-2010/.
...
The histogram of the log of deaths per capita is very normal, which would be perfect for analysis.

```{r scatterplot-race-income-cases-10}
...
```{r scatterplot-race-income-cases-1}

quants <- quantile(county_deaths$deathsPC, probs = c(0.99))
...
Above is the output of our final model with no interactions following the use of backwards selection.
...
After these transformations, the `cases` and `pop_2015` variables are much more normally distributed. The `perc_white` and `perc_black` variables are still skewed when log transformed, so we will work with the untransformed versions of them, and mark their failure of this condition in our conclusion.

After performing the transformations on the 2 variables, the final model needs to be updated to reflect this:
...
```{r full-interaction, warning = FALSE}
...
kable(tidy(full_interaction, format = "markdown"), digits = 4)
```

```{r full-interaction-selection, warning = FALSE}
interaction_model <- step(full_interaction, direction = "backward", trace = FALSE)
kable(tidy(interaction_model, format = "markdown"), digits = 4)
```

```{r final-interaction, warning=FALSE}

final <- lm(logDeathsPC ~ logCases + 
                    Rural.urban_Continuum_Code_2013 + PCTPOVALL_2018 +
                    log_pop_2015 + perc_white + perc_black + 
                      logCases*PCTPOVALL_2018 + logCases*perc_white +
                      logCases*perc_black +
                      PCTPOVALL_2018*perc_black +
                      PCTPOVALL_2018*log_pop_2015 + 
                      log_pop_2015*perc_black,
                    data = county_deaths)
...
Next, we will describe the coefficients for our interaction effect. 
...
The goal of this analysis was to explore the question:
`How does the COVID-19 pandemic disproportionately affect communities of color and poor Americans?`
...
In predicting from most variables in our `county_deaths` dataset, we created a full model that was then put through backward selection. After backward selection, the following predictors and coefficients remained:

```{r coefficients}
kable(tidy(final_model, format = "markdown"), digits = 7)
```

These coefficients predict log(deaths/capita). However, we further examined the interaction effects between these remaining predictors. Adding interaction effects between all remaining variables and again performing backward selection led to the predictors and coefficients in our final model:

```{r coefficients-2, warning=FALSE, message=FALSE}
kable(tidy(final, format = "markdown", exponentiate = TRUE), digits = 7)
```

We are able to evaluate the variables and interactions that remain as those that are most significant in predicting deaths per capita. As one would assume, the coefficient for `logCases` is relatively large, implying for every 1 increase in logCases, there will be a large increase in deaths per capita.

Most interestingly, and in accordance with our research question, the coefficient for `perc_black` is substantially larger than the other coefficients, being over 14 times larger than the other coefficents in predicting deaths per capita. This leads to the conclusion that a higher percent of a county made up of black inhabitants significantly increases the deaths per capita for the county. Further, when examining the interaction effects, any increase in `logCases`, `PCTPOVALL_2018`, and/or `log_pop_2015` increases the effect of `perc_black` on predicting deaths per capita. 
